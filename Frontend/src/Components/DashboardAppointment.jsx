        ...
        <div className="space-y-6">
                        ...
                        <form onSubmit={addAppointment} className="space-y-4">
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input value={form.doctor} onChange={e => setForm(p => ({ ...p, doctor: e.target.value }))} className=" w-full bg-[#192638] border border-white/10 focus:border-primary-500/60 text-white placeholder:text-white/30 rounded-xl px-4 outline-none transition-all duration-200 focus:ring-2 focus:ring-cyan-500/20 text-sm py-2.5" placeholder="Dr. Name" required />
                                </div>
                                <div>
                                    <label className="block text-xs text-white/40 mb-1.5">Specialty</label>
                                    <input value={form.specialty} onChange={e => setForm(p => ({ ...p, specialty: e.target.value }))} className=" w-full bg-[#192638] border border-white/10 focus:border-primary-500/60 text-white placeholder:text-white/30 rounded-xl px-4 py-3 outline-none transition-all duration-200 focus:ring-2 focus:ring-primary-500/20 text-sm py-2.5" placeholder="Cardiologist" required />
                                </div>
                            </div>
                            <div>
                                <label className="block text-xs text-white/40 mb-1.5">Hospital / Clinic</label>
                                <input value={form.hospital} onChange={e => setForm(p => ({ ...p, hospital: e.target.value }))} className=" w-full bg-[#192638] border border-white/10 focus:border-primary-500/60 text-white placeholder:text-white/30 rounded-xl px-4 py-3 outline-none transition-all duration-200 focus:ring-2 focus:ring-primary-500/20 text-sm py-2.5" placeholder="Hospital name" required />
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-xs text-white/40 mb-1.5">Date</label>
                                    <input type="date" value={form.date} onChange={e => setForm(p => ({ ...p, date: e.target.value }))} className=" w-full bg-[#192638] border border-white/10 focus:border-primary-500/60 text-white placeholder:text-white/30 rounded-xl px-4 py-3 outline-none transition-all duration-200 focus:ring-2 focus:ring-primary-500/20 text-sm py-2.5" required />
                                </div>
                                <div>
                                    <label className="block text-xs text-white/40 mb-1.5">Time</label>
                                    <input type="time" value={form.time} onChange={e => setForm(p => ({ ...p, time: e.target.value }))} className=" w-full bg-[#192638] border border-white/10 focus:border-primary-500/60 text-white placeholder:text-white/30 rounded-xl px-4 py-3 outline-none transition-all duration-200 focus:ring-2 focus:ring-primary-500/20 text-sm py-2.5" required />
                                </div>
                            </div>
                            <div>
                                <label className="block text-xs text-white/40 mb-1.5">Visit Type</label>
                                <input value={form.type} onChange={e => setForm(p => ({ ...p, type: e.target.value }))} className=" w-full  bg-[#192638] border border-white/10 focus:border-primary-500/60 text-white placeholder:text-white/30 rounded-xl px-4 py-3 outline-none transition-all duration-200 focus:ring-2 focus:ring-primary-500/20 text-sm py-2.5" placeholder="e.g. Follow-up, Routine Checkup" />
                            </div>
            ...
            <div className="space-y-3">
                {appointments.map(apt => {
                    return (
                        <div key={apt.id} className=' bg-surface-800/60 backdrop-blur-md border border-white/8 rounded-2xl p-5 border hover:border-white/15 transition-all'>
                            <div className="flex items-start gap-4 flex-wrap">
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    {apt.type && <div className="mt-2"><span className="badge bg-white/5 text-white/30 text-xs">{apt.type}</span></div>}
                                </div>
                            </div>
                        </div>
                    )
